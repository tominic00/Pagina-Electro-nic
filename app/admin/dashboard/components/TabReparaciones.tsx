...
import { Wrench, Users, Search, Plus, DollarSign, Smartphone, MessageCircle, Loader2, Trash2, Edit3, X, Lock, Banknote, Printer, Tag, FileClock, ShieldCheck, Star, Clock, Calendar, CheckCircle2, MapPin, Calculator } from "lucide-react"
...
export function TabReparaciones({
  ...
  const abrirNuevoRegistro = () => {
    setReparacionForm({ cliente_referencia: "", producto_id: "", nombre_servicio_manual: "", imei: "", color: "", diagnostico_falla: "", tecnico_id: "", costo_tecnico: 0, costo_repuesto: 0, total_trato: 0, monto_pagado: 0, metodo_pago: "Efectivo", estado: "Ingresado", tipo_contrasena: "Ninguna", contrasena_equipo: "", ubicacion_fisica: "Local" });
    setPatron([]);
    setNuevoPagoMonto("");
    setShowNuevaReparacion(true);
  }
  ...
  const repDelTecnico = todasLasReparaciones.filter((r: any) => r.tecnico_id === tecnicoHistorialId && r.estado !== "Ingreso Extra");
  ...
  const tecEquiposConTecnico = repDelTecnico.filter((r:any) => r.ubicacion_fisica === "Técnico").length;
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody className="divide-y divide-zinc-800/40">
                  {reparacionesFiltradas.filter((r:any) => r.estado !== "Ingreso Extra").map((rep: any) => {
                  ...
                  {reparacionesFiltradas.filter((r:any) => r.estado !== "Ingreso Extra").length === 0 && (<tr><td colSpan={5} className="p-16 text-center text-zinc-600 font-bold italic">No hay órdenes registradas bajo este filtro.</td></tr>)}
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
          ...
          <div className="xl:col-span-8 bg-[#161B22] border border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-zinc-800/40">
                {tecnicos.map((t: any) => {
                  const equiposDelTecnico = todasLasReparaciones.filter((r: any) => r.tecnico_id === t.id && r.estado !== 'Entregado' && r.estado !== "Ingreso Extra")
                  const manoDeObraPendiente = todasLasReparaciones.filter((r: any) => r.tecnico_id === t.id && r.pago_tecnico_estado !== 'Pagado' && r.estado !== "Ingreso Extra").reduce((a: number, r: any) => a + Number(r.costo_tecnico || 0), 0)
                  return (
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in zoom-in-95 duration-200">
          <div className="bg-[#121212] border border-zinc-800 w-full max-w-6xl rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
            <div className="p-5 border-b border-zinc-800 flex justify-between items-center bg-zinc-900/50">
              <div className="flex items-center gap-3"><div className="p-3 bg-purple-500/10 text-purple-500 rounded-xl border border-purple-500/20"><FileClock className="size-6"/></div><div><h3 className="text-xl font-black text-white">Rendimiento y Pagos</h3><p className="text-xs text-zinc-400 uppercase tracking-widest font-bold mt-0.5">{tecnicos.find((t:any)=>t.id === tecnicoHistorialId)?.nombre}</p></div></div>
            ...
            <div className="p-6 flex-1 overflow-y-auto space-y-6">
              <div className="flex flex-col sm:flex-row gap-4 items-end bg-zinc-950 p-4 rounded-2xl border border-zinc-800">
                <div className="flex-1"><label className="text-[10px] font-black uppercase text-zinc-500 block mb-1">Filtrar por Ubicación:</label><select value={techFiltroUbicacion} onChange={e=>setTechFiltroUbicacion(e.target.value)} className="w-full bg-zinc-900 border border-zinc-700 text-white text-sm rounded-xl px-3 py-2 outline-none focus:border-purple-500"><option value="todos">Todos los equipos</option><option value="Local">🏪 En el Local</option><option value="Técnico">🛠️ En poder del Técnico</option></select></div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-2xl"><span className="text-[10px] font-black uppercase text-zinc-500">Celulares con el Técnico</span><p className="text-3xl font-black text-amber-500 mt-1">{tecEquiposConTecnico}</p></div>
                <div className="bg-emerald-500/10 border border-emerald-500/20 p-5 rounded-2xl"><span className="text-[10px] font-black uppercase text-emerald-500">Reparados (En Rango)</span><p className="text-3xl font-black text-emerald-400 mt-1">{tecEquiposListos}</p></div>
                <div className="bg-blue-500/10 border border-blue-500/20 p-5 rounded-2xl"><span className="text-[10px] font-black uppercase text-zinc-400">Total a Pagar Seleccionado</span><p className="text-2xl font-black text-blue-400 mt-1">{formatARS(repDelTecnicoFiltradas.filter((r:any) => r.pago_tecnico_estado !== "Pagado").reduce((a:any, r:any) => a + Number(r.costo_tecnico || 0), 0))}</p></div>
                <div className="bg-amber-500/10 border border-amber-500/20 p-5 rounded-2xl flex flex-col justify-between">
                  <div><span className="text-[10px] font-black uppercase text-amber-500">Deuda Global (Total Histórico)</span><p className="text-2xl font-black text-amber-400 mt-1">{formatARS(tecDeudaManoObra)}</p></div>
                  <button onClick={() => { if(handleLiquidarPagosTecnico) handleLiquidarPagosTecnico(tecnicoHistorialId); else alert("Falta implementar handleLiquidarPagosTecnico"); }} disabled={tecDeudaManoObra === 0 || isSaving} className="mt-3 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-black font-black uppercase text-[10px] tracking-widest py-2 rounded-lg flex items-center justify-center gap-1 shadow-lg shadow-amber-500/20 transition-all"><CheckCircle2 className="size-3"/> Liquidar Deuda Global</button>
                </div>
              </div>
              <div className="bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden">
                <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-zinc-900 text-[9px] font-black uppercase tracking-widest text-zinc-500"><tr><th className="p-4 pl-6">Fecha / Cliente</th><th className="p-4">Falla Reparada</th><th className="p-4 text-center">Ubicación del Eq.</th><th className="p-4 text-right">A Pagar al Téc.</th><th className="p-4 text-center pr-6">Control de Pago</th></tr></thead>
                  <tbody className="divide-y divide-zinc-800/40">
                    {repDelTecnicoFiltradas.map((rep:any) => (
                      <tr key={rep.id} className="hover:bg-zinc-800/20">
                        <td className="p-4 pl-6"><span className="font-bold text-white text-sm block">{rep.cliente_referencia}</span><span className="text-[10px] text-zinc-500 font-bold block mt-0.5">{new Date(rep.created_at).toLocaleDateString()}</span></td>
                        <td className="p-4 text-zinc-300 font-medium text-xs">{rep.nombre_producto?.replace("Servicio: ", "")}</td>
                        <td className="p-4 text-center"><select value={rep.ubicacion_fisica || "Local"} onChange={e => handleUpdateInline(rep.id, 'ubicacion_fisica', e.target.value)} className={cn("text-[9px] font-black uppercase tracking-widest px-2 py-1.5 rounded-lg border outline-none cursor-pointer transition-all", rep.ubicacion_fisica === 'Técnico' ? 'bg-amber-500/10 text-amber-500 border-amber-500/30' : 'bg-zinc-950 text-zinc-400 border-zinc-800')}><option value="Local">🏪 Local</option><option value="Técnico">🛠️ Técnico</option></select></td>
                        <td className="p-4 text-right font-black text-white">{formatARS(rep.costo_tecnico)}</td>
                        <td className="p-4 text-center pr-6"><button onClick={() => handleUpdateInline(rep.id, 'pago_tecnico_estado', rep.pago_tecnico_estado === 'Pagado' ? 'Pendiente' : 'Pagado')} className={cn("px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border transition-all shadow-md w-28", rep.pago_tecnico_estado === "Pagado" ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20 hover:bg-red-500/10 hover:text-red-500 hover:border-red-500/30" : "bg-amber-500 text-black border-amber-400 hover:bg-amber-400")}>{rep.pago_tecnico_estado === "Pagado" ? "✅ Abonado" : "💰 Pagar Esta"}</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className={cn("bg-[#121212] border border-zinc-800 w-full rounded-3xl shadow-2xl overflow-hidden relative flex flex-col max-h-[95vh] transition-all", reparacionForm.id ? "max-w-6xl" : "max-w-2xl")}>
            ...
            <div className="p-6 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/40">
              <div className="flex items-center gap-3">
                <div className="p-2.5 bg-purple-600/10 rounded-xl border border-purple-500/20">{reparacionForm.id ? <ShieldCheck className="size-5 text-purple-500"/> : <Wrench className="size-5 text-purple-500"/>}</div>
                <div><h3 className="text-lg font-black text-white">{reparacionForm.id ? `Panel de Control: Ticket #${reparacionForm.id.slice(0,8).toUpperCase()}` : "Ingreso al Taller"}</h3><p className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">{reparacionForm.id ? `Ingresado el ${new Date(reparacionForm.created_at).toLocaleDateString()}` : "Generar nueva orden de servicio"}</p></div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4 p-4 bg-zinc-900/50 rounded-2xl border border-zinc-800/50">
                    <div>
                      ...
                      <select value={reparacionForm.tipo_contrasena || "Ninguna"} onChange={e => { setReparacionForm({...reparacionForm, tipo_contrasena: e.target.value, contrasena_equipo: ""}); setPatron([]); }} className="w-full rounded-xl border border-zinc-800 bg-[#121212] p-2.5 text-sm text-white outline-none focus:border-purple-500 mb-3"><option value="Ninguna">Sin Contraseña</option><option value="PIN">PIN Numérico</option><option value="Alfanumerica">Alfanumérica</option><option value="Patron">Patrón Táctil</option></select>
                      ...
                      <select value={reparacionForm.ubicacion_fisica || "Local"} onChange={e => setReparacionForm({...reparacionForm, ubicacion_fisica: e.target.value})} className="w-full rounded-xl border border-zinc-800 bg-[#121212] p-2.5 text-sm text-white outline-none focus:border-purple-500 mb-3"><option value="Local">🏪 En el Local</option><option value="Técnico">🛠️ En poder del Técnico</option></select>
                      <label className="text-[10px] font-bold text-zinc-400 mb-1 block">Técnico Asignado</label>
                      <select value={reparacionForm.tecnico_id || ""} onChange={e => setReparacionForm({...reparacionForm, tecnico_id: e.target.value})} className="w-full rounded-xl border border-zinc-800 bg-[#121212] p-2.5 text-sm text-white outline-none focus:border-purple-500"><option value="">-- Sin Asignar --</option>{tecnicos.map((t: any) => <option key={t.id} value={t.id}>{t.nombre}</option>)}</select>
                    </div>
                    <div className="flex flex-col items-center justify-center">
                      {reparacionForm.tipo_contrasena === "Patron" ? (
                        <div className="bg-[#161B22] border border-zinc-800 rounded-2xl p-4 w-full flex flex-col items-center shadow-inner relative"><div className="grid grid-cols-3 gap-3">{[1,2,3,4,5,6,7,8,9].map(num => { const stepIndex = patron.indexOf(num); const isSelected = stepIndex !== -1; return (<button key={num} type="button" onClick={() => !isSelected && setPatron([...patron, num])} className={cn("size-10 rounded-full border-2 flex items-center justify-center transition-all", isSelected ? "bg-purple-600 border-purple-400 shadow-[0_0_15px_rgba(147,51,234,0.5)]" : "bg-zinc-900 border-zinc-700 hover:border-zinc-500")}>{isSelected && <span className="text-white font-black text-xs">{stepIndex + 1}</span>}</button>)})}</div><button type="button" onClick={() => setPatron([])} className="mt-3 text-[10px] font-bold text-red-400 uppercase">Borrar Patrón</button></div>
                      ) : (
                ...
                {!reparacionForm.id && (
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div><label className="text-[10px] font-bold text-zinc-400 mb-1 block">Seña Inicial (ARS)</label><div className="relative"><DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-emerald-500" /><input type="number" value={reparacionForm.monto_pagado || ""} onChange={e => setReparacionForm({...reparacionForm, monto_pagado: Number(e.target.value)})} className="w-full rounded-xl border border-emerald-900/30 bg-emerald-500/5 p-3 pl-9 text-sm font-bold text-emerald-400 outline-none focus:border-emerald-500" /></div></div>
                    <div><label className="text-[10px] font-bold text-zinc-400 mb-1 block">Costo de Repuesto (Local)</label><div className="relative"><DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-orange-500" /><input type="number" value={reparacionForm.costo_repuesto || ""} onChange={e => setReparacionForm({...reparacionForm, costo_repuesto: Number(e.target.value)})} className="w-full rounded-xl border border-orange-900/30 bg-orange-500/5 p-3 pl-9 text-sm font-bold text-orange-400 outline-none focus:border-orange-500" /></div></div>
                    <div><label className="text-[10px] font-bold text-zinc-400 mb-1 block">A Pagar al Téc. (Mano Obra)</label><div className="relative"><DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-red-500" /><input type="number" value={reparacionForm.costo_tecnico || ""} onChange={e => setReparacionForm({...reparacionForm, costo_tecnico: Number(e.target.value)})} className="w-full rounded-xl border border-red-900/30 bg-red-500/5 p-3 pl-9 text-sm font-bold text-red-400 outline-none focus:border-red-500" /></div></div>
                  </div>
                )}
              </div>
                ...
                <div className="space-y-6 flex flex-col h-full">
                  ...
                  <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-5 shadow-inner">
                    ...
                    <div className="space-y-4">
                      ...
                      {reparacionForm.mensaje_cliente && (<div className="bg-purple-600/10 border border-purple-500/20 p-3 rounded-xl"><p className="text-[10px] font-black uppercase text-purple-400 mb-1">Mensaje del cliente:</p><p className="text-sm text-purple-100 italic">"{reparacionForm.mensaje_cliente}"</p></div>)}
                      {reparacionForm.rating_estrellas > 0 && (<div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded-xl flex flex-col items-center"><div className="flex gap-1 mb-1">{[1,2,3,4,5].map(s => <Star key={s} className={cn("size-4", reparacionForm.rating_estrellas >= s ? "fill-amber-500 text-amber-500" : "text-zinc-700")} />)}</div><p className="text-xs text-amber-200 font-bold text-center">"{reparacionForm.resena_texto}"</p></div>)}
                    </div>
                  </div>
                  ...
                  <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 shadow-lg flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4 pb-4 border-b border-zinc-800">
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-emerald-500 mb-1 block uppercase tracking-widest">Ganancia Neta (Taller)</label>
                        <div className="text-xl font-black text-emerald-400 bg-emerald-500/10 rounded-xl px-3 py-2 border border-emerald-500/30">
                          {formatARS(Number(reparacionForm.total_trato) - (Number(reparacionForm.costo_tecnico || 0) + Number(reparacionForm.costo_repuesto || 0)))}
                        </div>
                      </div>
                      <div><label className="text-[10px] font-bold text-zinc-400 mb-1 block">Costo Repuesto (ARS)</label><div className="relative"><DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-orange-500" /><input type="number" value={reparacionForm.costo_repuesto || ""} onChange={e => setReparacionForm({...reparacionForm, costo_repuesto: Number(e.target.value)})} className="w-full rounded-xl border border-orange-900/30 bg-orange-500/5 p-2.5 pl-8 text-sm font-bold text-orange-400 outline-none focus:border-orange-500" /></div></div>
                      <div><label className="text-[10px] font-bold text-zinc-400 mb-1 block">Mano de Obra (ARS)</label><div className="relative"><DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-red-500" /><input type="number" value={reparacionForm.costo_tecnico || ""} onChange={e => setReparacionForm({...reparacionForm, costo_tecnico: Number(e.target.value)})} className="w-full rounded-xl border border-red-900/30 bg-red-500/5 p-2.5 pl-8 text-sm font-bold text-red-400 outline-none focus:border-red-500" /></div></div>
                    </div>

                    <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-800 mb-4">
                      <div className="flex justify-between items-center mb-3">
                        <span className="text-sm font-bold text-zinc-400">Total ya Cobrado:</span>
                      ...
                      <div className="flex gap-2 items-end mb-4 bg-zinc-900 p-3 rounded-xl border border-zinc-700 shadow-inner">
                        <div className="flex-1">
                          ...
                          <div className="relative">
                            <DollarSign className="absolute left-2 top-1/2 -translate-y-1/2 size-3 text-white" />
                            <input type="number" value={nuevoPagoMonto} onChange={e=>setNuevoPagoMonto(e.target.value)} placeholder="0.00" className="w-full bg-zinc-950 border border-zinc-600 text-white text-sm font-bold rounded-lg px-3 py-2 pl-6 outline-none focus:border-purple-500" />
                          </div>
                        </div>
                        <button type="button" onClick={() => {
                          if(!nuevoPagoMonto || Number(nuevoPagoMonto) <= 0) return;
                          setReparacionForm({...reparacionForm, monto_pagado: Number(reparacionForm.monto_pagado) + Number(nuevoPagoMonto)});
                          setNuevoPagoMonto("");
                        }} className="bg-emerald-500 hover:bg-emerald-400 text-black font-black uppercase text-[10px] tracking-widest px-4 py-2.5 rounded-lg transition-colors shadow-lg shadow-emerald-500/20 active:scale-95">
                          Acreditar
                        </button>
                      </div>

                      <div className="flex gap-2 justify-end mb-4">
                        <span className="text-[10px] text-zinc-500 font-bold self-center mr-2">Sumar Rápido:</span>
