...
import { ShoppingCart, Search, DollarSign, History, Plus, X, PackageOpen, Loader2, Printer, FileText, CheckCircle2, Users, Tag } from "lucide-react"
import  supabase  from "@/lib/supabase"
import { cn } from "@/lib/utils"
import jsPDF from "jspdf"
import autoTable from "jspdf-autotable" // 🚀 Importación corregida para evitar el error del PDF
...
export function TabVentas({ usuarioActual }: { usuarioActual: any }) {
  ...
  // ESTADOS DE DESCUENTO / RECARGO GLOBAL
  ...
  // Filtrar stock para el buscador
  ...
  if (tipoAjuste === "monto") {
    montoAjusteGlobal = valorAjuste
  } else {
    montoAjusteGlobal = totalVentaBase * (valorAjuste / 100)
  }
  ...
  const generarPDF = (tipoDocumento: "PRESUPUESTO" | "REMITO OFICIAL", listaItems: any[], nombreCliente: string, totalFacturado: number, ajuste: number) => {
    try {
      const doc = new jsPDF()
      ...
      if (tipoDocumento === "PRESUPUESTO") {
        ...
        doc.text("* Los precios expresados en este presupuesto están sujetos a modificaciones sin previo aviso.", 14, finalY + 25)
        doc.text("* Este documento no compromete reserva de stock.", 14, finalY + 30)
      }
  ...
  // 🚀 OBTENER NOMBRE DEL CLIENTE FINAL
  ...
  const handlePresupuesto = () => {
    ...
    generarPDF("PRESUPUESTO", carrito, getNombreCliente(), totalVentaFinal, montoAjusteGlobal)
  }
  ...
  const handleCerrarVentaMultiple = async () => {
    ...
    try {
      ...
      // 1. Si es cliente nuevo, lo guardamos primero en la base de clientes
      if (clienteId === "NUEVO") {
      ...
      // Repartimos el ajuste global (descuento o recargo) proporcionalmente entre todos los ítems para mantener las analíticas limpias
      ...
      // 4. Generar Remito PDF
      generarPDF("REMITO OFICIAL", carrito, nombreCliente, totalVentaFinal, montoAjusteGlobal)
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2">
      ...
      <div className="p-6 border-b xl:border-b-0 xl:border-r border-zinc-800 bg-[#161B22] flex flex-col h-full">
        ...
        {/* 1. SELECCIÓN DE CLIENTE INTELIGENTE */}
        ...
        {/* 2. AGREGAR AL CARRITO (Ajuste Individual) */}
          ...
          <form onSubmit={handleAgregarAlCarrito} className="space-y-4">
            ...
            <div className="flex gap-3 items-end">
              <div className="flex-1">
                <label className="text-[10px] font-black uppercase text-emerald-500 block mb-1">Precio Unitario (Podés editarlo para descuento)</label>
                <div className="relative">
                  <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-emerald-500" />
                  <input required type="number" value={precioItem} onChange={e => setPrecioItem(e.target.value)} placeholder="0" className="w-full bg-emerald-500/5 border border-emerald-500/30 text-emerald-400 font-black rounded-xl pl-9 pr-3 py-3 outline-none focus:border-emerald-400 transition-all" />
                </div>
              </div>
        ...
        {/* 4. AJUSTE GLOBAL Y TOTALES */}
        {carrito.length > 0 && (
          <>
            {/* Panel de Ajuste Global */}
            <div className="flex flex-col sm:flex-row gap-3 items-center bg-zinc-900 border border-zinc-800 p-4 rounded-2xl mb-4">
               <div className="flex-1 w-full">
                 <label className="text-[10px] font-black uppercase text-zinc-400 flex items-center gap-1.5 mb-2"><Tag className="size-3"/> Ajuste Global a la Cuenta</label>
                 <div className="flex gap-2">
                   <select value={tipoAjuste} onChange={(e) => setTipoAjuste(e.target.value as any)} className="bg-zinc-950 border border-zinc-700 text-white rounded-lg px-2 py-2 text-xs outline-none focus:border-sky-500">
                     <option value="monto">Monto (U$D)</option>
                     <option value="porcentaje">Porcentaje (%)</option>
                   </select>
                   <input type="number" value={ajusteGlobal} onChange={e => setAjusteGlobal(e.target.value ? Number(e.target.value) : "")} placeholder="Ej: -50 o 50" className="w-full bg-zinc-950 border border-zinc-700 text-white font-bold rounded-lg px-3 py-2 text-sm outline-none focus:border-sky-500 transition-all" />
                 </div>
                 <p className="text-[9px] text-zinc-500 mt-1.5">Usá números <span className="text-red-400 font-bold">negativos</span> para descuentos y <span className="text-sky-400 font-bold">positivos</span> para recargos.</p>
               </div>
            </div>

            <div className="bg-emerald-500/10 border border-emerald-500/20 p-5 rounded-2xl animate-in slide-in-from-bottom-4">
              <div className="flex justify-between items-end mb-1">
                <p className="text-[10px] font-black uppercase text-emerald-600 tracking-widest">Subtotal</p>
                <p className="text-sm font-bold text-emerald-500/70">U$D {totalVentaBase.toFixed(2)}</p>
              </div>
              
              {montoAjusteGlobal !== 0 && (
                <div className="flex justify-between items-end mb-2 border-b border-emerald-500/20 pb-2">
                  <p className="text-[10px] font-black uppercase text-emerald-600 tracking-widest">Ajuste Global</p>
                  <p className={cn("text-sm font-bold", montoAjusteGlobal < 0 ? "text-red-400" : "text-sky-400")}>
                    {montoAjusteGlobal > 0 ? "" : "+"}U$D {montoAjusteGlobal.toFixed(2)}
                  </p>
                </div>
              )}
          ...
          {ventas.map(v => (
            <div key={v.id} className="bg-zinc-900/50 hover:bg-zinc-900 border border-zinc-800 p-4 rounded-2xl flex justify-between items-center transition-colors">
              ...
              <div className="text-right flex items-center gap-3">
                ...
                <button 
                  onClick={() => generarPDF("REMITO OFICIAL", [v], v.cliente, Number(v.monto_vendido_usd), 0)} 
                  className="bg-zinc-800 hover:bg-sky-500 hover:text-black text-zinc-400 p-2.5 rounded-xl transition-all"
                  title="Descargar Comprobante PDF de este equipo"
                >
