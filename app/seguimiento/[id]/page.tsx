...
export default function SeguimientoCliente({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchOrden = async () => {
      const { data } = await supabase.from("ventas_b2b").select("*").eq("id", params.id).single()
  ...
  const aceptarPresupuesto = async () => {
    setAceptando(true)
    const { error } = await supabase.from("ventas_b2b").update({ presupuesto_aceptado: true }).eq("id", params.id)
  ...
  const enviarMensaje = async () => {
    ...
    setAceptando(true)
    const { error } = await supabase.from("ventas_b2b").update({ mensaje_cliente: mensajeTexto }).eq("id", params.id)
  ...
  const enviarResena = async () => {
    ...
    setAceptando(true)
    const { error } = await supabase.from("ventas_b2b").update({ rating_estrellas: rating, resena_texto: resenaTexto }).eq("id", params.id)
  ...
  if (!orden) return <div className="min-h-screen bg-[#09090b] flex items-center justify-center text-white font-bold">Orden no encontrada.</div>
      ...
      <div className="p-4 space-y-6 overflow-y-auto w-full md:max-w-md flex-1 pb-10">
        ...
        <div className="flex flex-col gap-1 w-[90%]">
          ...
          <div className="bg-zinc-800 text-sm text-zinc-200 p-4 rounded-2xl rounded-tl-sm shadow-md">
            ¡Hola, <b>{orden.cliente_referencia.split(' ')[0]}</b>! 👋 Ingresamos tu <b>{orden.nombre_producto?.replace("Servicio: ", "")}</b>.<br/><br/><b>Falla reportada:</b> {orden.diagnostico_falla}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1 w-[90%]">
          <span className="text-[10px] text-zinc-500 font-bold ml-2">Electro·nic</span>
          <div className="bg-zinc-800 text-sm text-zinc-200 p-4 rounded-2xl rounded-tl-sm shadow-md">
            El presupuesto estimado es de <b>$ {Number(orden.total_trato).toLocaleString("es-AR")}</b>.<br/><br/>
        ...
        {orden.mensaje_cliente && (
          <div className="flex flex-col gap-1 w-[85%] self-end ml-auto items-end">
            ...
            {!orden.presupuesto_aceptado && (
              <button onClick={aceptarPresupuesto} className="mt-1 text-[10px] font-bold text-purple-400 border border-purple-500/30 px-3 py-1.5 rounded-full">Dar el OK Final al Presupuesto</button>
            )}
          </div>
        )}
        ...
        {orden.estado === "Entregado" && (
           <div className="flex flex-col gap-1 w-full pt-4 border-t border-zinc-800">
             {!orden.rating_estrellas ? (
               <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-2xl flex flex-col items-center shadow-lg">
                 ...
                 <button onClick={enviarResena} disabled={aceptando} className="w-full bg-amber-500 text-black font-black uppercase tracking-widest py-3 rounded-xl">Enviar Reseña</button>
               </div>
             ) : (
