...
export function TabInventario({
  ...
  // 🚀 ESTADO PARA ABRIR EL NUEVO MODAL GIGANTE
  const [showFormModal, setShowFormModal] = useState(false)
  const [mediaUrlInput, setMediaUrlInput] = useState("")

  // Cuando le damos a editar desde la tabla, abrimos el modal
  useEffect(() => {
  ...
  }, [showFormModal, formData.descripcion === ""])
  ...
  // 🚀 LÓGICA DE ETIQUETAS (Subcategorías)
  const handleAddEtiqueta = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      const val = e.currentTarget.value.trim();
      if (val && !(formData.subcategorias || []).includes(val)) {
        setFormData({ ...formData, subcategorias: [...(formData.subcategorias || []), val] });
        e.currentTarget.value = '';
      }
    }
  }

  const removeEtiqueta = (tagToRemove: string) => {
    setFormData({ ...formData, subcategorias: (formData.subcategorias || []).filter((t: string) => t !== tagToRemove) });
  }

  // 🚀 LÓGICA DE GALERÍA MULTIMEDIA
  ...
  // Sincronizar la imagen principal vieja con la galería si sube archivo tradicional
  useEffect(() => {
  ...
  const handleExportCSV = () => {
    ...
    const rows = productosFiltrados.map(p => [ p.id, `"${p.nombre}"`, `"${p.categoria}"`, p.moneda || "ARS", p.costo || 0, p.precio_minorista ?? p.precio, p.precio_mayorista ?? p.precio, p.stock || 0 ])
  ...
  const abrirNuevoRegistro = () => {
    setEditingId(null);
    setFormData({ nombre: "", precio: 0, costo: 0, stock: 0, categoria: "", imagen_url: "", moneda: "ARS", visible_web: true, subcategorias: [], galeria: [] });
  ...
  return (
    <div className="flex flex-col animate-in fade-in duration-500 text-left w-full h-full">
      ...
      {/* 🚀 HEADER Y ACCIONES GENERALES (AHORA OCUPA TODO EL ANCHO) */}
      <div className="mb-6 flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <div className="flex gap-2 flex-wrap sm:flex-nowrap">
            ...
            {/* 🚀 BOTÓN GIGANTE PARA NUEVO PRODUCTO */}
      ...
      {/* 🚀 TABLA PRINCIPAL (AHORA OCUPA EL 100% DE LA PANTALLA) */}
      ...
      {/* 🚀 MODAL MAGISTRAL DE PRODUCTO (A pantalla dividida) */}
      {showFormModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in zoom-in-95 duration-200">
          <div className="bg-[#121212] border border-zinc-800 w-full max-w-6xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
            ...
            <div className="p-6 overflow-y-auto flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              {/* COLUMNA 1: Datos Base y Precios */}
              <div className="lg:col-span-4 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input required type="text" value={formData.nombre || ""} onChange={e => setFormData({...formData, nombre: e.target.value})} className="mt-1 w-full rounded-xl border border-zinc-800 bg-zinc-950 p-2.5 text-sm text-white outline-none focus:border-purple-500" placeholder="Ej: Display iPhone 13 Pro Max" />
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] font-black uppercase tracking-wider text-zinc-500">Categoría Raíz</label>
                      <input required type="text" list="categorias-sugeridas" value={formData.categoria || ""} onChange={e => setFormData({...formData, categoria: e.target.value})} placeholder="Ej: Repuestos..." className="mt-1 w-full rounded-xl border border-zinc-800 bg-zinc-950 p-2.5 text-sm text-white outline-none focus:border-purple-500" />
                    ...
                    <div>
                      ...
                      <select value={formData.moneda || "ARS"} onChange={e => setFormData({...formData, moneda: e.target.value})} className="mt-1 w-full rounded-xl border border-zinc-800 bg-zinc-950 p-2.5 text-sm font-bold text-white outline-none focus:border-purple-500">
                  ...
                  {/* ETIQUETAS (Subcategorías Múltiples) */}
                  <div>
                    ...
                    <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-2 min-h-[44px] flex flex-wrap gap-2 items-center focus-within:border-purple-500 transition-colors">
                      {(formData.subcategorias || []).map((tag: string) => (
                        <span key={tag} className="bg-purple-500/20 text-purple-300 border border-purple-500/30 px-2 py-1 rounded-md text-[10px] font-bold flex items-center gap-1">
                          {tag} <button type="button" onClick={() => removeEtiqueta(tag)} className="hover:text-white"><X className="size-3"/></button>
                        </span>
                      ))}
                      <input type="text" onKeyDown={handleAddEtiqueta} placeholder={(!formData.subcategorias || formData.subcategorias.length === 0) ? "Escribí y tocá Enter..." : "Añadir..."} className="bg-transparent border-none outline-none text-sm text-white flex-1 min-w-[80px]" />
                    </div>
                  </div>
                </div>

                <div className="space-y-4 rounded-xl border border-zinc-800 bg-zinc-950/50 p-4 shadow-inner">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-emerald-400 border-b border-zinc-800 pb-1 flex items-center gap-1"><DollarSign className="size-3.5"/> Precios e Inventario</h4>
                  
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="text-[10px] font-bold text-zinc-500">Costo Base</label><div className="relative mt-1"><span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs font-bold text-zinc-500">{simboloFormActual}</span><input required type="number" min="0" step="0.01" value={formData.costo === 0 && formData.costo.toString() !== "0" ? "" : formData.costo} onChange={e => setFormData({...formData, costo: Number(e.target.value)})} className="w-full rounded-lg border border-zinc-800 bg-zinc-900 p-2 pl-7 text-sm font-bold text-white outline-none focus:border-emerald-500" /></div></div>
                    <div><label className="text-[10px] font-bold text-zinc-500">Stock Inicial</label><input required type="number" min="0" value={formData.stock === 0 && formData.stock.toString() !== "0" ? "" : formData.stock} onChange={e => setFormData({...formData, stock: Number(e.target.value)})} className="mt-1 w-full rounded-lg border border-zinc-800 bg-zinc-900 p-2 text-sm text-white outline-none focus:border-purple-500 disabled:opacity-50" disabled={!!editingId}/></div>
                  </div>

                  <div><label className="text-[10px] font-bold text-zinc-500">Precio Público (Mostrador/Web)</label><div className="relative mt-1"><span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs font-bold text-emerald-500">{simboloFormActual}</span><input required type="number" min="0" step="0.01" value={formData.precio_minorista ?? formData.precio ?? ""} onChange={e => setFormData({...formData, precio_minorista: Number(e.target.value), precio: Number(e.target.value)})} className="w-full rounded-lg border border-zinc-800 bg-[#161B22] p-2.5 pl-7 text-sm font-black text-white outline-none focus:border-emerald-500" placeholder="0.00" /></div></div>
                  <div><label className="text-[10px] font-bold text-zinc-500 flex items-center gap-1"><Users className="size-3"/> Precio Gremio (Técnicos)</label><div className="relative mt-1"><span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs font-bold text-emerald-500/50">{simboloFormActual}</span><input required type="number" min="0" step="0.01" value={formData.precio_mayorista ?? formData.precio ?? ""} onChange={e => setFormData({...formData, precio_mayorista: Number(e.target.value)})} className="w-full rounded-lg border border-zinc-800 bg-[#161B22] p-2 pl-7 text-sm font-bold text-zinc-300 outline-none focus:border-emerald-500" placeholder="0.00" /></div></div>
                </div>
              ...
              <div className="lg:col-span-8 flex flex-col gap-6">
                ...
                <div className="flex flex-col border border-zinc-800 rounded-xl bg-zinc-950 overflow-hidden flex-1 min-h-[300px] shadow-inner">
                  ...
                  {/* Hoja Editable Blanca estilo Documento */}
            ...
            <div className="p-6 border-t border-zinc-800 bg-[#161B22] flex justify-end gap-3 shrink-0">
              ...
              <button onClick={handleSave} disabled={isSaving || isUploading} className="px-10 py-3.5 flex items-center justify-center rounded-xl bg-purple-600 text-xs font-black tracking-widest uppercase text-white hover:bg-purple-500 transition-all shadow-lg shadow-purple-600/20 active:scale-95 disabled:opacity-50">
      ...
      {/* 🚀 MODAL INSTRUCCIONES IMPORTACIÓN MASIVA (QUEDA IGUAL) */}
      {showImportModal && (
